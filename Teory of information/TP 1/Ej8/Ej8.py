...
#I calculate with a bound, not with the number of iterations

def estandar_error (p, n):
    return math.sqrt((p*(1-p)/n))

#Idem 1.
def simulate_experiment (cota_error):
    ...
    while True:
        ...
        p1 = results['Two simples']/n
        p2 = results['Two difficult']/n
        p3 = results['One and one']/n
        p4 = results['Difficult after simple']/n

        ee_p1 = estandar_error(p1,n)
        ee_p2 = estandar_error(p2,n)
        ee_p3 = estandar_error(p3,n)
        ee_p4 = estandar_error(p4,n)

        if(n>5):
            if(ee_p1 < cota_error and ee_p2 < cota_error and ee_p3 < cota_error and ee_p4 < cota_error):
                break


    print(f"Without replacement, results after {n} n: ")

    two_simples_prob = f"{(results['Two simples']/n) * 100:.2f}"
    two_difficult_prob = f"{(results['Two difficult']/n) * 100:.2f}"
    one_and_one_prob = f"{(results['One and one']/n) * 100:.2f}"
    difficult_after_simple_prob = f"{(results['Difficult after simple']/n) * 100:.2f}"
